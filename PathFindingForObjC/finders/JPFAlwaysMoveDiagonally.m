...
@implementation JPFAlwaysMoveDiagonally
...
- (NSArray *)findNeighbors:(PFNode *)node withGrid:(PFGrid *)grid {
	...
	if (parent) {
		...
		if (dx != 0 && dy != 0) {
			...
			PFNode *c3Node = [grid getNodeAtX:(x + dx) andY:(y + dy)];
	...
	// return all neighbors
		...
		NSArray *neighborNodes = [grid getNeighborsWith:node diagonalMovement:DiagonalMovement_Always];
